...
[Apache TsFile](https://tsfile.apache.org/) is a columnar file format designed specifically for time-series data. It natively supports timestamps, device tags, and measurement fields, enabling efficient time-range queries.
...
You can filter by time range, which is pushed down to TsFile's internal time index:
...
> [!TIP]
> For more details, check out the [how to load tabular datasets from TsFile files](tabular_load#tsfile-files) guide.
